...
For this fork, the main training entrypoint is `dps.py` (depth + custom deep-COLMAP workflow).  
To run the optimizer, use:

```shell
python dps.py -s <path to COLMAP or NeRF Synthetic dataset> -m <output_dir>
```

<details>
<summary><span style="font-weight: bold;">Command Line Arguments for dps.py (base 3DGS options)</span></summary>
  ...
  #### --iterations
  Number of total iterations to train for, ```20_000``` by default.
  #### --ip
  ...
  #### --save_iterations
  Space-separated iterations at which the training script saves the Gaussian model, ```7000 20000 <iterations>``` by default.
...
```shell
python dps.py -s <path to COLMAP or NeRF Synthetic dataset> -m <output_dir> --eval # Train with train/test split
...
The default pipeline is designed around undistorted camera models (`SIMPLE_PINHOLE` / `PINHOLE`). In this fork, COLMAP models with common distortion types (e.g. `SIMPLE_RADIAL`, `RADIAL`, `OPENCV`, `OPENCV_FISHEYE`, `FULL_OPENCV`, `FOV`) are also accepted for loading; distortion coefficients are ignored by the loader and focal terms are used to compute FoV. For best geometric consistency, we still recommend running `convert.py` to produce undistorted inputs. Optionally, you can use ImageMagick to resize the undistorted images. This rescaling is similar to MipNeRF360, i.e., it creates images with 1/2, 1/4 and 1/8 the original resolution in corresponding folders. To use them, please first install a recent version of COLMAP (ideally CUDA-powered) and ImageMagick. Put the images you want to use in a directory ```<location>/input```.
```
<location>
|---input
    |---<image 0>
...
Then you can add the following parameter to use the sparse adam optimizer when running `dps.py`:
...
### Custom deep COLMAP + feature initialization flow (dps.py)

This repository adds a `dps.py` training path with two different feature-based initialization options:

1. **Full COLMAP-style deep SfM reconstruction (recommended)**  
   Enable with `--deep_colmap_reconstruction`.  
   Internally (`utils/deep_colmap_runtime.py`):
   - Builds image pairs (`exhaustive` or `window` mode).
   - Runs deep local feature extraction and matching through `hloc`.
   - Runs geometric verification + sparse reconstruction (triangulation + bundle adjustment) with `pycolmap`.
   - Handles different `hloc` output layouts, then prepares a valid source folder at:
     `.../deep_colmap/source_for_dps` containing `images/` and `sparse/0/*`.
   - `dps.py` automatically switches `source_path` to this reconstructed model before training starts.

2. **Depth-assisted deep feature seed augmentation (optional)**  
   Enable with `--feature_seed_points`.  
   Internally (`utils/feature_seed_runtime.py`):
   - Extracts deep descriptors per frame (default backbone: `resnet18`, optional `resnet50` via `--feature_deep_backbone`).
   - Matches descriptors between nearby frames (mutual NN + cosine threshold).
   - Backprojects matched pixels to 3D using depth priors + known camera poses.
   - Keeps only geometrically consistent matches and merges them into the initial point cloud.
   - Optionally exports tracks as COLMAP text model files (`cameras.txt`, `images.txt`, `points3D.txt`).

**Important:** the two options are different by design.  
`--deep_colmap_reconstruction` is full SfM triangulation/BA.  
`--feature_seed_points` is depth-assisted point augmentation on top of an existing model.

#### Dependencies for full deep COLMAP flow

```bash
pip install pycolmap "git+https://github.com/cvg/Hierarchical-Localization.git"
```

#### Minimal command (full deep COLMAP + dps training)

```bash
python dps.py \
  -s <dataset_root> \
  -m <output_dir> \
  --deep_colmap_reconstruction \
  --deep_colmap_extractor disk \
  --deep_colmap_matcher disk+lightglue \
  --deep_colmap_pair_mode exhaustive \
  --deep_colmap_overwrite
```

#### Recommended `dps.py` commands for depth + custom deep-COLMAP

1) **COLMAP + precomputed depth maps (`-d`)**
```bash
python dps.py \
  -s <dataset_root> \
  -m <output_dir> \
  -d <depths_dir> \
  --depth_clip_mode auto \
  --disable_viewer
```

2) **Custom deep-COLMAP reconstruction + Gaussian training (recommended)**
```bash
python dps.py \
  -s <dataset_root> \
  -m <output_dir> \
...
3) **Deep-COLMAP + runtime Depth-Anything priors + stronger depth supervision**
```bash
python dps.py \
  -s <dataset_root> \
  -m <output_dir> \
  ...
  --depth_l1_start_iter 5000 \
  --depth_loss_scale 1.0 \
  --disable_viewer
```

#### Key `dps.py` arguments for this workflow

- **Core input/output**
  - `-s/--source_path`: dataset root (expects `images/` and `sparse/0/`).
  - `-m/--model_path`: output training directory.
  - `-i/--images`: image subfolder (default: `images`).
  - `-d/--depths`: depth folder used with depth supervision (`depth_params.json` expected).

- **Custom deep COLMAP reconstruction**
  - `--deep_colmap_reconstruction`: enable full deep-feature SfM pre-step.
  - `--deep_colmap_extractor`: HLOC extractor config (e.g. `disk`).
  - `--deep_colmap_matcher`: HLOC matcher config (e.g. `disk+lightglue`).
  - `--deep_colmap_pair_mode`: `exhaustive` or `window`.
  - `--deep_colmap_pair_window`: neighbor window size when using `window`.
  - `--deep_colmap_overwrite`: overwrite previous deep-COLMAP outputs.

- **Depth priors and losses**
  - `--depth_anything`: compute monocular priors during training.
  - `--mv_depth`: compute multiview priors during training.
  - `--depth_clip_mode auto`: robust clipping from depth percentiles.
  - `--depth_loss_pred`: `inverse` (default) or `depth`.
  - `--depth_loss_mask`: `none`, `semantic`, `alpha`, `semantic_alpha`.
  - `--depth_l1_weight_init`, `--depth_l1_weight_final`, `--depth_l1_start_iter`, `--depth_loss_scale`: main depth-weight schedule.

- **Initialization enrichment (optional)**
  - `--feature_seed_points`: add depth-assisted feature seed points.
  - `--feature_seed_pair_window`, `--feature_match_sim_thresh`, `--feature_deep_backbone`: tune deep feature seeding.
  - `--depth_seed_*`: control direct depth seed point generation.

- **Training/runtime**
  - `--iterations` (default `20000`), `--optimizer_type sparse_adam`, `--antialiasing`.
  - `--disable_viewer` for headless runs.
  - `--save_iterations`, `--checkpoint_iterations`, `--start_checkpoint`.
  - `--plot_losses`, `--log_file`, `--log_interval` for tracking.

### How depth is used in dps.py

Depth is used in multiple stages, not only as a single loss term:

1. **Depth prior sources**
   - Dataset depth maps (`-d/--depths` with `depth_params.json`).
   - Runtime monocular priors (`--depth_anything`).
   - Runtime multi-view priors (`--mv_depth`).

2. **Depth sanitization and conversion**
   - Priors are sanitized to remove NaN/Inf/non-positive values.
   - Converted to inverse depth when required.
   - When `--depth_anything` is enabled, a COLMAP-scaled depth point cloud is exported to
     `<model_path>/depth_anything_pointcloud/depth_anything_points.ply` by default
     (override with `--depth_anything_pointcloud_dir`, density with `--depth_anything_pointcloud_stride`).
   - Depth artifacts (maps + point clouds) are exported by default for active depth sources
     to `<model_path>/depth_artifacts/` (disable with `--no_export_depth_artifacts`).

3. **Adaptive clipping**
   - `--depth_clip_mode auto` computes robust clipping bounds from prior percentiles.

4. **Initialization with depth seeds**
   - Optional depth-seed point generation from masks/depth maps.
   - Includes scale alignment options, optional RANSAC, and safeguards when COLMAP is too sparse.

5. **Depth supervision during optimization**
   - Main depth-order/L1 supervision.
   - Optional multi-scale order loss.
   - Optional smoothness, magnitude consistency, and range regularization.
   - Optional semantic/alpha masking of depth loss (`--depth_loss_mask`).

6. **Scheduling and weighting**
   - Independent start modes for mask and depth (`iter` or `plateau`).
   - Warmup and global scaling (`--depth_loss_warmup_steps`, `--depth_loss_scale`).
   - Detailed logging of raw depth loss and weighted depth loss.

If no valid depth priors are loaded and runtime priors are disabled, depth loss will remain near zero.
...
### Anti-aliasing
We added the EWA Filter from [Mip Splatting](https://niujinshuchong.github.io/mip-splatting/) in our codebase to remove aliasing. It is disabled by default but you can enable it by adding `--antialiasing` when training on a scene using `dps.py` or rendering using `render.py`. Antialiasing can be toggled in the SIBR viewer, it is disabled by default but you should enable it when viewing a scene trained using `--antialiasing`.
